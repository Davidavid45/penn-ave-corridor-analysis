...
**Note on `impact_zone_comparison.csv`:** This file contains pre-computed crash rate aggregates (crashes per mile by zone) and was created manually from pipeline outputs. It is not auto-generated by any script — if crash data is re-pulled, this file must be manually updated.
...
- `impact_zone_comparison.csv` is manually maintained — if crash data is re-pulled, crash rate figures must be recalculated and the file updated by hand.
